...
namespace MCD.Audio
{
    public class MCDAudio
    {
		public static AudioClip LoadWavFromEmbeddedResource(string name)
		{
			...
			using (Stream stream = assembly.GetManifestResourceStream(resourceName))
			{
				if (stream == null)
				{
					Debug.LogError("Unable to load resource stream: " + resourceName);
					return null;
				}
				...
				// Convert the byte array to a WAV AudioClip
				AudioClip clip = WavUtility.ToAudioClip(buffer, 0, name);
				return clip;
			}
		}
        

        public static AudioSource PlayMusic(GameObject playingNow, AudioClip audioResource, float previousClipFadeOutTime = 1f, bool fadePrevious = true, float vol = 1f, bool looped = false)
		{
			if (playingNow != null && fadePrevious)
			{
				SHAM.Instance.FadeoutAndLeave(playingNow, previousClipFadeOutTime);
			}

			AudioSource audioClipEffects = SHAM.Instance.PlayClip(audioResource, null, vol, null, 128, false, looped);

			return audioClipEffects;
			// AudioSource.gameobject
		}

		private class WavUtility
		{
